...
from app.services import llm, notifications, pipeline
...
def dashboard(
    ...
    return DashboardOut(
        ...
        # The buckets, the reason vocabulary and the empty state belong to
        # services/pipeline. This route renders them; `app.cli status` and the
        # offline HTML report render the same call, so the three cannot drift.
        buckets=pipeline.buckets(db, user, agent_settings, since),
        empty_state=pipeline.empty_state(db, user, count(JobMatch)),
        recent_activity=[
            {
                "seq": entry.seq,
